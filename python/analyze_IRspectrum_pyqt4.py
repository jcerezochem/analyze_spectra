...
def helptext():
    print """
       A python application to analyze IR spectra from g09 logs/fchk
  ...
  * Reference Spectrum
  ...
  ---------------------------------
    ...
    def load_sticks(self,stick_type):
        ...
        self.axes2.set_xlim([xmin-150.0,xmax+150.0])
    ...
    def load_ir_sticks(self):
        ...
        self.axes.set_xlim([xmin-150.0,xmax+150.])
    ...
    def set_axis_labels(self):
        ...
        self.axes.set_xlabel(self.xaxis_units,fontsize=16)
        self.axes.set_ylabel('Stick '+self.data_type,fontsize=16)
        self.axes2.set_ylabel(self.data_type,fontsize=16)
        #self.axes.tick_params(direction='out',top=False, right=False)
    ...
    def update_scale(self):
        ...
        # Update selected
        if self.active_tr:
            i = self.active_tr.ind-1
            x = self.xstick[i]*self.scale
            data = self.selected.get_segments()
            data[0][0][0] = x
            data[0][1][0] = x
            self.selected.set_segments(data)
            # Update text
            self.active_tr.freqsc = self.active_tr.freq*self.scale
            self.analysis_box.setText(self.active_tr.info())
        # (labels should be updated!)
        # ...
        # Update convoluted spectra (redoing the conv)
    ...
    def load_experiment_spc(self,x,y):
        ...
        self.spectrum_ref = self.axes2.plot(x,y,'-',color='red',label="Ref")
        if self.data_type == 'Transmittance':
            self.axes.set_ylim ([0.,100.])
            self.axes2.set_ylim([0.,100.])
        elif not fixaxes:
    ...
    def select_stick(self,event):
        ...
        Select a given stick transiton by clicking with the mouse, and do:
         ...
         2) Highlight is right button is clicked root
        """
        x = event.mouseevent.xdata
        y = event.mouseevent.ydata

        #Now set data and label positions
        stick_x = [ self.modes[i].freq     for i in event.ind ]
        stick_y = [ self.modes[i].irintens for i in event.ind ]
        distances = np.hypot(x-stick_x, y-stick_y)
        indmin = distances.argmin()
        dataind = event.ind[indmin]
        
        # Highlight or add label
        tr = self.modes[dataind]
        if event.mouseevent.button == 3:
            ...
            self.modes[dataind].freqsc = self.modes[dataind].freq*self.scale
            self.modes[dataind].info()
            self.add_label(tr)
    ...
    def on_press_key(self, event):
        ...
        index = self.modes.index(self.active_tr)
        index += inc
        
        # Get next transition. If out-of-bounds, simply 
        # clean the panel
        try:
            self.active_tr = self.modes[index]
            self.set_stick_marker()
        except IndexError:
            self.del_stick_marker()
    ...
    def add_label(self,tr):
        ...
        stick_x = self.xstick[tr.ind-1]
        stick_y = self.ystick[tr.ind-1]
    ...
    def update_scale_from_textbox(self):
        str = unicode(self.scalebox.text())
    ...
    def set_stick_marker(self):
        ...
        tr = self.active_tr 
        stick_x = self.xstick[tr.ind-1]*self.scale
        stick_y = self.ystick[tr.ind-1]
        ...
        if self.data_type == 'Intensity':
            ...
            self.selected  = self.axes.vlines([stick_x], [100.0], [stick_y], linewidths=3,
                                    color='yellow', visible=True, alpha=0.7)
        self.canvas.draw()
    ...
    def update_data_type(self):
        ...
        if current_data_type != self.data_type:
            ...
            if current_data_type == "Intensity":
                ...
                #--------
                if self.data_type == "Transmittance":
                    self.ystick = 100.0 * 10.**(-self.ystick*self.conc)
                    y = self.ystick.copy()
                    data = self.stickspc.get_segments()
                    for i in range(len(data)):
                        data[i][0][1] = 100.0
                        # Stick transmittance is attenuated
                        data[i][1][1] = (y[i]/100.0)**(0.1) * 100.00
                    self.stickspc.set_segments(data)
                # Conv.
                #--------
                x = self.spectrum_sim[0].get_xdata()
                y = self.spectrum_sim[0].get_ydata()
                if self.data_type == "Transmittance":
                    y = 100.0 * 10.**(-y*self.conc)
                #elif self.data_type == "Lineshape":
            ...
            elif current_data_type == "Transmittance":
                # Sticks
                #--------
                if self.data_type == "Intensity":
                    self.ystick = -np.log10(self.ystick/100.0)/self.conc
                    y = self.ystick.copy()
                    data = self.stickspc.get_segments()
                    for i in range(len(data)):
                        data[i][0][1] = 0
...
0-->0       - 0-0 transition
        ...
        if tr_select:
            ...
            stick_y = []
            for tr in tr_select:
                stick_x.append(self.xstick[tr.ind-1])
                stick_y.append(self.ystick[tr.ind-1])
                msg = msg+"\n"+tr.info()
    ...
    def create_main_frame(self):
        ...
        self.select_data_type.addItems(["Intensity","Transmittance"]) #,"Lineshape"])
        self.select_data_type.currentIndexChanged.connect(self.update_data_type)
        
        self.select_xaxis_units = QComboBox()
        self.select_xaxis_units.addItems(["Wavenumber(cm-1)"]) #,"Energy(eV)","Wavelength(nm)"])
        ...
        # Place here the scale things
        hbox_scalebox = QHBoxLayout()
        hbox_scalebox.addWidget(scale_label)
        hbox_scalebox.addWidget(self.scalebox)
        ...
        vbox_slider.addLayout(hbox_scalebox)
        # Clean button
...
# CONVOLUTION
def convolute(spc_stick,npoints=5000,hwhm=10.0,broad="Lor",input_bins=False):
    """
...
def export_xmgrace(filename,ax,sticks,labs,ax2=None,specs=None):
    ...
    * labs        dict            : labels in xmgr format (arg: labels as annotation Class)
    ...
    k += 1
    # Load sticks (as explict "line" spectrum (not with bar))
    data = sticks.get_segments()
    x = np.array([ [ data[j][i][0] for i in [0,1,0] ] for j in range(len(data)) ]).flatten()
    y = np.array([ [ data[j][i][1] for i in [0,1,0] ] for j in range(len(data)) ]).flatten()
...
def get_args():
 ...
 ----------------------------------------
           IR analyzer
   A GUI to analyze vibrational spectra
 ----------------------------------------
        """
        helptext()
        print "    Options:"
